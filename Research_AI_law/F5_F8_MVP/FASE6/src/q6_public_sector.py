...
def run_q6(fm: pd.DataFrame, config: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    questions_cfg = config.get("questions", {})
    q6_cfg = questions_cfg.get("Q6_public_sector", {})
    outcomes = q6_cfg.get("primary_outcomes", [])
    ...
    for outcome in outcomes:
        ...
        for predictor in x1_core:
            ...
            design = ModelDesign(
                ...
                controls=x2_minimal,
                model_family="linear_score_model_or_fractional_if_rescaled",
                analysis_role="primary_continuous_or_score",
            )
