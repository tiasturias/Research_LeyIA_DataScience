...
def test_feature_matrix_and_membership_contracts(phase5_results):
    feature_matrix = pd.read_csv(FASE5_OUTPUTS / "feature_matrix_mvp.csv")
    membership = pd.read_csv(FASE5_OUTPUTS / "analysis_sample_membership.csv")

    assert feature_matrix.shape[0] == 43
    assert feature_matrix["iso3"].is_unique
    assert membership.shape[0] == 43
    assert membership["iso3"].is_unique
    assert "split" not in feature_matrix.columns
    assert "split" not in membership.columns
...
def test_auditable_excel_has_expected_sheets(phase5_results):
    ...
    assert set(workbook.sheetnames) == set(EXPECTED_EXCEL_SHEETS)
